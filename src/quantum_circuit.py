...
class LDESolver:
    ...
    def entanglement(self):
        ...
        # print(self.circuit)
        self.circuit.append(
            XGate().control(num_ctrl_qubits=3, ctrl_state='100'),
            [self.anc_reg2[0], self.anc_reg2[1], self.anc_reg2[2], self.work_qubit_reg[-1]]
        )
        self.circuit.append(
            XGate().control(num_ctrl_qubits=3, ctrl_state='110'),
            [self.anc_reg2[0], self.anc_reg2[1], self.anc_reg2[2], self.work_qubit_reg[-1]]
        )
        self.circuit.append(
            XGate().control(num_ctrl_qubits=3, ctrl_state='101'),
            [self.anc_reg2[0], self.anc_reg2[1], self.anc_reg2[2], self.work_qubit_reg[-1]]
        )
        self.circuit.append(
            XGate().control(num_ctrl_qubits=3, ctrl_state='111'),
            [self.anc_reg2[0], self.anc_reg2[1], self.anc_reg2[2], self.work_qubit_reg[-1]]
        )
    ...
    def encode(self):
        ...
        S1_circuit.h(3)
        # Vs1 Vs2
        S1_circuit.unitary(self.v_S1_U, [0, 1, 2], label='VS1')
        S1_gate = S1_circuit.to_gate().control(1, ctrl_state='0')

        S2_circuit = QuantumCircuit(4)
        S2_circuit.x(3)
        S2_circuit.unitary(self.v_S2_U, [0, 1, 2], label='VS2')
    ...
    def create_VS_WS(self, t):
        ...
        v_S2 = np.array(
            [[np.sqrt(self.e(t, 1, order, noSum=True, index=1)),
              np.sqrt(self.e(t, 2, order, noSum=True, index=2)),
              np.sqrt(self.e(t, 3, order, noSum=True, index=3)),
              np.sqrt(self.e(t, 3, order, noSum=True, index=4)),
              np.sqrt(self.e(t, 3, order, noSum=True, index=5)),
              np.sqrt(self.e(t, 3, order, noSum=True, index=6)),
              np.sqrt(self.e(t, 3, order, noSum=True, index=7)),
              0]]
        )
